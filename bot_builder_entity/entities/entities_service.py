...
class EntitiesService:
    ...
    def update(self, update_entity_dto: UpdateEntityDto) -> bool:
        ...
        entity = self.session.exec(select(Entity).where(
            Entity.id == entity_raw['id'])).first()
        ...
        for option_raw in entity_raw['options']:
            if option_raw['id']:
                option = self.session.exec(select(Option).where(
                    Option.id == option_raw['id'])).first()

                if option_raw['deleted']:
                    self.session.exec(delete(Synonym).where(
                        Synonym.option_id == option.id))
                    ...
                    for synonym_raw in option_raw['synonyms']:
                        if synonym_raw['deleted'] and synonym_raw['id']:
                            synonym = self.session.exec(select(Synonym).where(
                                Synonym.id == synonym_raw['id'])).first()
    ...
    def delete(self, delete_entity_dto: DeleteEntityDto) -> bool:
        ...
        for entity_id in delete_entity_dto['ids']:
            entity = self.session.exec(
                select(Entity).where(Entity.id == entity_id)).first()
            options = self.session.exec(select(Option).where(
                Option.entity_id == entity.id)).all()
